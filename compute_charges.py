...
def computeCharges(pdb_filename, vertices, names):
    """
    takes a protonated pdb file and surface vertices from MSMS to calculate the charges in each vertex
    :param pdb_filename: path to protonted (preferably) pdb file nam
    :param vertices: vertices calculated by MSMS see compute_msms.py
    :param names: names of the vertices as above
    :return: returns h bond potential for each vertex
    """
...
def computeChargeHelper(atom_name, res, v):
    """
    this is directly copied from MASIF for each atom on a vertex determine whether it is a h bond acceptor or donor
    and the angle of the Hbond
    :param atom_name: name of the atom from computeMSMS
    :param res: aa name
    :param v: not sure what this is
    :return: returns charge of a vertex
    """
...
# or 4 vertices)
def assignChargesToNewMesh(new_vertices, old_vertices, old_charges, feature_interpolation):
    """
    for transferring the caclcuated charges to simplified mesh, this basically runs a kdtree to find the closest
    aas and their respective properties and assigs and average to the new vertex
    :param new_vertices: from fixmesh.py
    :param old_vertices: from pymesh.from_mesh
    :param old_charges: from .computeCharges (or compute_hydrophobicicy.computeHydrophobicity)
    :param feature_interpolation:
    :return:
    """
